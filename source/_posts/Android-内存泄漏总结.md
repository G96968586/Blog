...
* MAT分析heap的总内存占用大小来初步判断是否存在泄露
	...
	![](https://gw.alicdn.com/tps/TB1pdf2LVXXXXXeXXXXXXXXXXXX-690-514.jpg)
...
* MAT分析hprof来定位内存泄露的原因所在
	...
	![](https://gw.alicdn.com/tps/TB1r2zZLVXXXXcHXXXXXXXXXXXX-640-167.jpg)

	B)分析持有此类对象引用的外部对象

	![](https://gw.alicdn.com/tps/TB17XvOLVXXXXbiXFXXXXXXXXXX-640-90.png)

	C)分析这些持有引用的对象的GC路径

	![](https://gw.alicdn.com/tps/TB10yTwLVXXXXaRapXXXXXXXXXX-640-278.png)

	D)逐个分析每个对象的GC路径是否正常

	![](https://gw.alicdn.com/tps/TB1CWTQLVXXXXamXFXXXXXXXXXX-640-90.png)
...
* MAT对比操作前后的hprof来定位内存泄露的根因所在
	...
	![](https://gw.alicdn.com/tps/TB1p1rULVXXXXbyXpXXXXXXXXXX-525-212.png)

	D）打开第二个HPROF 文件然后重做步骤2和3.

	E）切换到Compare Basket view, 然后点击Compare the Results (视图右上角的红色”!”图标)。

	![](https://gw.alicdn.com/tps/TB1p0zKLVXXXXX.XVXXXXXXXXXX-640-98.png)

	F）分析对比结果

	![](https://gw.alicdn.com/tps/TB1lwDMLVXXXXcUXFXXXXXXXXXX-640-115.png)
...
[Android怎样coding避免内存泄露](http://www.atatech.org/articles/44198)

[支付宝钱包Android内存治理](http://www.atatech.org/articles/44197/?frm=mail_daily&uid=130319)
